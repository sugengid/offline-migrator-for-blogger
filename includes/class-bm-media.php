...
class BMIG_Media {
	...
	public function run( $albums_path, $blog_name = '' ) {
		...
		if ( ! is_dir( $albums_path ) ) {
			return new WP_Error( 'bmig_media_no_dir', __( 'Albums folder not found.', 'sugeng-offline-migrator-for-blogger' ) );
		}
	...
	private function import_candidate_album( $url, array $entries, $preferred, array &$stats ) {
		...
		if ( $attachment_id ) {
			...
			if ( ! $attachment_id ) {
				$stats['import_errors'][ $url ] = __( 'Failed to import file to the media library.', 'sugeng-offline-migrator-for-blogger' );
				return;
			}
	...
	private function import_candidate_external( $url, array &$stats ) {
		...
		if ( ! self::is_external_url_safe( $url ) ) {
			$stats['import_errors'][ $url ] = __( 'URL blocked: host not allowed.', 'sugeng-offline-migrator-for-blogger' );
			return;
		}
		...
		if ( is_wp_error( $tmp ) ) {
			$stats['import_errors'][ $url ] = sprintf(
				...
				__( 'Download failed: %s', 'sugeng-offline-migrator-for-blogger' ),
		...
		if ( empty( $filetype['type'] ) || 0 !== strpos( $filetype['type'], 'image/' ) ) {
			wp_delete_file( $tmp );
			$stats['import_errors'][ $url ] = __( 'Downloaded file is not a supported image.', 'sugeng-offline-migrator-for-blogger' );
			return;
		}
		...
		if ( is_wp_error( $attachment_id ) ) {
			...
			$stats['import_errors'][ $url ] = sprintf(
				...
				__( 'Failed to save to the media library: %s', 'sugeng-offline-migrator-for-blogger' ),
